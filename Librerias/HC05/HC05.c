                        ...
                        GPIO_TypeDef* Power_GPIOx, uint16_t Power_Pin)
{
	...
	uint32_t at_baudrates[] = {9600, 38400};	// 38400 es más común en muchos HC-05
	...
	for(int baud_idx = 0; baud_idx < 2; baud_idx++)
	{
		...
		uint32_t actual_baud = HC05_HUART->Instance->BRR;  // Ver en debugger

		// Limpiar cualquier basura en el buffer despuÃ©s de reconfigurar
		HAL_Delay(100);
		HC05_FlushUART();
 ...
 * @return HC05_Status_t Estado de la operación
 *
 * @note DespuÃ©s de llamar esta función, el módulo estará listo para
...
HC05_Status_t HC05_EnterDataMode(void)
{
	...
	// Limpiar buffer despuÃ©s de reconfigurar
	HAL_Delay(100);
	HC05_FlushUART();
...
/**
 * @brief Envía datos a travÃ©s de Bluetooth
 *
...
/**
 * @brief Recibe datos a travÃ©s de Bluetooth
 *
...
{
	uint8_t dummy;
	// Leer hasta que no haya mÃƒÂ¡s datos o timeout
